...
- **bootstrap.css**
  ...
  - **Contains:**
    This stylesheet was supplied with the original bootstrap template and hasn't been modified or cleaned. Potentially contains a lot of redundant styles. The base layout used is 12 column size-sensitive. See comments inside of the file for more information.
...
- **checkout.css**
  ...
  - **Contains:**
    A small collection of styles developed for preorder page particularly. Potentially some of the styles from style.css could be moved here to improve loading time.
...
- **`/images/news/`**
 Custom backgrounds for news links. See `press.html` info on how to add them.
...
- **`/js/script.js`**
 Main Javascript file which initializes and plugins, functions and variables used throughout the whole website. Plugins available are:
    ...
    - **`isotope`**
     Isotope layout (currently not in use)
    - **`radio`**
